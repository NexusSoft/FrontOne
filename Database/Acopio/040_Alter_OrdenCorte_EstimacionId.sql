...
-- Liga opcional a la Estimación de la que salió el precio de esta Orden de Corte. Nullable: no
-- toda Orden parte de una Estimación (las ya existentes quedan en NULL). Sin CASCADE/SET NULL
-- (regla dura del proyecto) — una Estimación referenciada por una Orden no se puede borrar.
IF NOT EXISTS (SELECT 1 FROM sys.columns WHERE object_id = OBJECT_ID('Acopio.OrdenCorte') AND name = 'EstimacionId')
BEGIN
    ALTER TABLE Acopio.OrdenCorte ADD EstimacionId INT NULL;
END
GO

IF NOT EXISTS (SELECT 1 FROM sys.foreign_keys WHERE name = 'FK_Acopio_OrdenCorte_Estimacion')
BEGIN
    ALTER TABLE Acopio.OrdenCorte
        ADD CONSTRAINT FK_Acopio_OrdenCorte_Estimacion FOREIGN KEY (EstimacionId) REFERENCES Acopio.Estimacion (Id);
END
GO
...
BEGIN
    ...
    INSERT INTO Acopio.OrdenCorte
        (Folio, Fecha, AcuerdoCorteId, ProductorId, HuertaId, FloracionId, RegistroSagarpa, VariedadId,
         ...
         PuntoReunion, Observaciones, Cancelado, CajaCampoId, EstimacionId)
    VALUES
        (@Folio, @Fecha, @AcuerdoCorteId, @ProductorId, @HuertaId, @FloracionId, @RegistroSagarpa, @VariedadId,
         @PagarCorteACardCode, @PagarCorteANombre, @TransportistaCardCode, @TransportistaNombre, @PrecioAcarreo,
         @NoCandado, @CajasEntregadas, @JefeCuadrillaCardCode, @JefeCuadrillaNombre,
