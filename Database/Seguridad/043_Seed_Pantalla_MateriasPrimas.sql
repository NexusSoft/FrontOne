...
SELECT m.Id, 'MateriasPrimas', 'Catálogo de materias primas'
FROM Seguridad.Modulo m
WHERE m.Nombre = 'Catalogos'
  AND NOT EXISTS (SELECT 1 FROM Seguridad.Pantalla p WHERE p.ModuloId = m.Id AND p.Nombre = 'MateriasPrimas');
GO

-- Se mantienen los 4 permisos estándar (Consultar/Crear/Editar/Eliminar) por consistencia con el
-- resto de pantallas del sistema, aunque el form de listado no tenga botón Eliminar: las materias
-- primas solo se crean/eliminan vía sincronización con SAP, nunca desde la UI.
