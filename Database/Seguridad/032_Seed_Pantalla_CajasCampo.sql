...
SELECT m.Id, 'CajasCampo', 'Catálogo de cajas de campo (por color)'
FROM Seguridad.Modulo m
WHERE m.Nombre = 'Catalogos'
  AND NOT EXISTS (SELECT 1 FROM Seguridad.Pantalla p WHERE p.ModuloId = m.Id AND p.Nombre = 'CajasCampo');
GO
