...
SELECT m.Id, 'Huertas', 'Catálogo de huertas'
FROM Seguridad.Modulo m
WHERE m.Nombre = 'Catalogos'
  AND NOT EXISTS (SELECT 1 FROM Seguridad.Pantalla p WHERE p.ModuloId = m.Id AND p.Nombre = 'Huertas');
GO
