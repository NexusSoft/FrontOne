...
-- Pantalla TiposPago del módulo Acopio: ya existía TipoPagoEditarForm (alta rápida vía "+" de
-- TiposCorteForm) pero nunca tuvo botón propio en el Ribbon ni pantalla/permiso registrados.
-- Ahora se agrega botón directo en el nuevo grupo "Acopio" del Ribbon (página Catálogos).

INSERT INTO Seguridad.Pantalla (ModuloId, Nombre, Descripcion)
SELECT m.Id, v.Nombre, v.Descripcion
FROM Seguridad.Modulo m
CROSS JOIN (VALUES
    ('TiposPago', 'Catálogo de tipos de pago')
