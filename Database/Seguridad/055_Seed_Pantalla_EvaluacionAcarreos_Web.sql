...
-- Registra la consulta web en el módulo Acopio existente.
INSERT INTO Seguridad.Pantalla (ModuloId, Nombre, Descripcion)
SELECT m.Id, 'EvaluacionAcarreos', 'Página "Evaluación Acarreos" del sitio FrontOne.Web'
FROM Seguridad.Modulo m
WHERE m.Nombre = 'Acopio'
  AND NOT EXISTS (SELECT 1 FROM Seguridad.Pantalla p WHERE p.ModuloId = m.Id AND p.Nombre = 'EvaluacionAcarreos');
GO
...
WHERE r.Nombre = 'Administrador'
  AND m.Nombre = 'Acopio'
  AND p.Nombre = 'EvaluacionAcarreos'
  AND NOT EXISTS (
