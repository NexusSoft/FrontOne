...
SELECT m.Id, 'LineasProduccion', 'Catálogo de líneas de producción (mercado destino del Lote)'
FROM Seguridad.Modulo m
WHERE m.Nombre = 'Catalogos'
  AND NOT EXISTS (SELECT 1 FROM Seguridad.Pantalla p WHERE p.ModuloId = m.Id AND p.Nombre = 'LineasProduccion');
GO
