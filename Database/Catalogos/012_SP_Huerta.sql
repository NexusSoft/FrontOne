...
BEGIN
    ...
    INSERT INTO Catalogos.Huerta
        (Nombre, ProductorId, Ubicacion, PoblacionId, Municipio, EstadoId, ProductoId,
         EncargadoNombre, EncargadoTelefono, Observaciones, RegistroSagarpa, CertificadoGlobalGap,
         RegistroFda, NumeroGlobalGap, Superficie, Altura, NumeroArboles, EdadArboles,
         SistemaRiegoId, PorcentajeMecanizacion, StatusHuertaId, FechaCambioStatus, FechaVencimiento, Activo)
    VALUES
        (@Nombre, @ProductorId, @Ubicacion, @PoblacionId, @Municipio, @EstadoId, @ProductoId,
         @EncargadoNombre, @EncargadoTelefono, @Observaciones, @RegistroSagarpa, @CertificadoGlobalGap,
         @RegistroFda, @NumeroGlobalGap, @Superficie, @Altura, @NumeroArboles, @EdadArboles,
...
CREATE OR ALTER PROCEDURE Catalogos.sp_Huerta_Eliminar
    @Id INT
AS
BEGIN
    SET NOCOUNT ON;

    DELETE FROM Catalogos.Huerta WHERE Id = @Id;
END
GO

CREATE OR ALTER PROCEDURE Catalogos.sp_Huerta_ExisteRegistroSagarpa
    @RegistroSagarpa NVARCHAR(50),
    @IdExcluir       INT = NULL
AS
BEGIN
    SET NOCOUNT ON;
    ...
    SELECT CAST(CASE WHEN EXISTS (
        SELECT 1 FROM Catalogos.Huerta
        WHERE RegistroSagarpa = @RegistroSagarpa
