...
## Bug transversal: cambiar de tema DX rompía los márgenes/padding de `site.css` (2026-09-12)

Reportado en las pantallas de Acopio (`OrdenesCorte.razor`, `Estimacion.razor`), pero el bug vivía en el mecanismo de temas (`FrontOne.Web/wwwroot/js/theme-switcher.js`), no en esas páginas — afecta a cualquier CSS propio del sitio que dependa de ganar un empate de cascada contra el tema DX activo.

**Causa raíz**: en la carga inicial, el tema viene de `<link id="dx-theme-default">` (`App.razor`), declarado ANTES que `<link href="css/site.css?v=...">` — con ese orden, `site.css` carga después y gana los empates de especificidad. `frontOneCargarHojas` (en `theme-switcher.js`) inyectaba cada tema nuevo con `document.head.appendChild(link)`, es decir, siempre al FINAL del `<head>` — después de `site.css`. Desde el primer cambio de tema (el `<link id="dx-theme-default">` se deshabilita y ya no se vuelve a usar), el orden queda invertido para siempre: el CSS del tema pasa a ganar los empates contra `site.css`, y no se corrige ni volviendo a elegir el tema original (también se inyecta con `appendChild`).

**Fix**: `frontOneCargarHojas` ahora localiza el `<link>` de `site.css` y usa `document.head.insertBefore(link, siteCssLink)` en vez de `appendChild` — así `site.css` queda SIEMPRE de último en el `<head>`, sin importar cuántas veces se cambie de tema. Verificado con `getBoundingClientRect`/inspección del orden de `<link>` en el navegador: con el tema Cerulean activo (deliberadamente distinto del default), `site.css` aparece antes que `bootstrap-external.bs5.min.css` en el DOM, y el margen del panel de detalle de Órdenes de Corte se mantiene idéntico al del tema original.

Construido por Codex (`codex-rescue`) con el diagnóstico y el fix exacto ya especificados en el prompt — cambio de una sola función. Revisé el `git diff` (una función modificada, el resto del archivo intacto) antes de aceptarlo.
