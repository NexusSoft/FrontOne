...
## Iteración 4 — la caja pasa a Producción solo cuando el camión se destara

Petición del usuario: *"si en recepcion no esta desatarado no se puede crear el lote"* y *"cuando se pone en desatarado se cambia la caja de campo a produccion"*. Son dos reglas que cuelgan del mismo flag ya existente, `Recepcion.RecepcionFruta.CamionDestarado`.

Razón de negocio: mientras el camión no se destara falta la pesada en vacío, así que ni los kilos ni el conteo de cajas de esa Recepción son definitivos. Hasta entonces la Recepción no está cerrada y no debe mover inventario ni poder conformar un Lote.

**Movimientos de Almacenes** (`RecepcionFrutaService.RegistrarMovimientoEntradaAsync`): se agregó `!recepcion.CamionDestarado` a la guarda de salida temprana. Si no está destarado, el método hace el `EliminarPorOrigen` de siempre y **se sale sin insertar nada** — las cajas siguen contando como `EnCampo`. Al marcar "Camión destarado" y guardar, `ActualizarAsync` vuelve a llamar el mismo método (patrón borra-y-reinserta ya existente) y recién ahí se registran los 3 movimientos y la caja pasa a `Produccion`. No hizo falta lógica nueva de transición: el patrón borra-y-reinserta ya hacía el trabajo, solo faltaba la condición.

**Limpieza de paso**: se eliminó un overload muerto `RegistrarMovimientoEntradaAsync(int, RecepcionFruta)` que había quedado del modelo de 2 cuentas (iteración 1). No lo llamaba nadie —los 3 call sites usan la versión de 1 parámetro— pero si se hubiera vuelto a conectar habría insertado `Cuenta = ""` y reventado contra el `CHECK` de la columna. Compilaba porque `Cuenta` tiene `= string.Empty` por defecto, así que el error solo habría aparecido en runtime.

**Corrección de datos reales**: la Recepción Folio 0000020 (Id 14) tenía `CamionDestarado = 0` pero ya tenía sus 3 movimientos registrados con la regla vieja. Se borraron con `EliminarPorOrigen('Recepcion', 14)` — exactamente lo que hará el sistema solo al volver a guardarla. Saldos ROJA antes: `Existencia 700 / EnCampo 1600 / Produccion 700`; después: `Existencia 600 / EnCampo 1900 / Produccion 500` (suman 3000, que es la compra neta manual acumulada — cuadra).

**Resolución de la inconsistencia del Lote 21**: la Recepción 0000020 (Id 14) estaba dentro del Lote 0000021 sin estar destarada, cosa que la regla nueva prohíbe. El usuario decidió marcarla como destarada (en vez de sacarla del Lote). Se aplicó en la BD dentro de una transacción con `SET XACT_ABORT ON` + `TRY/CATCH`, derivando todos los valores de los datos reales (nunca hardcodeados): `UPDATE CamionDestarado = 1`, luego `EliminarPorOrigen('Recepcion', 14)` + los 3 inserts vía `sp_MovimientoCajaCampo_Insertar` — exactamente la misma secuencia que corre `RegistrarMovimientoEntradaAsync` al guardar desde la app. Saldos ROJA: antes `Existencia 600 / EnCampo 1900 / Produccion 500`, después `Existencia 700 / EnCampo 1600 / Produccion 700` (suman 3000, cuadra con la compra neta manual).

**Duda abierta sobre qué cuenta "En Producción"**: el usuario esperaba ver 900 (3 Recepciones × 300 cajas) y el dashboard muestra 700. La diferencia son las 200 cajas que llegaron **vacías** (100 en la Recepción 0000018 + 100 en la 0000020), que hoy van a `Existencia` y no a `Produccion`, siguiendo la definición original del usuario (*"las cajas vacias son las que llegaron pero no tiene materia prima"*). Queda pendiente confirmar si "En Producción" debe contar solo las cajas con fruta (comportamiento actual) o todas las que llegaron al empaque — no se cambió nada porque son dos definiciones de negocio distintas y el usuario todavía no eligió.
