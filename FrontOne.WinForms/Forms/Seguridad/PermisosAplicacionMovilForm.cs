...
// Permisos de la aplicación móvil (FrontOne.Android): tabla propia Seguridad.MovilPermiso, mismo
// patrón que Seguridad.ReportePermiso — independiente de Seguridad.Pantalla/Permiso genérico, así
// que guardar aquí no toca ni requiere reconstruir los permisos de escritorio del rol.
public partial class PermisosAplicacionMovilForm : XtraForm
{
    ...
    public PermisosAplicacionMovilForm(RolService rolService, MovilPermisoService movilPermisoService)
        : this()
    {
    ...
    {
        var matriz = await _movilPermisoService.ObtenerMatrizAsync(rolId);
        ...
        if (_gridView.Columns["PantallaCodigo"] is { } colPantalla)
        {
    ...
    private async void BtnGuardar_Click(object? sender, EventArgs e)
    {
        ...
        var filasDto = _filas
            .Select(f => new MovilPermisoPantallaDto(f.PantallaCodigo, string.Empty, f.Consultar, f.Crear, f.Modificar, f.Eliminar))
            .ToList();

        await _movilPermisoService.GuardarAsync(rolId, filasDto);
